...
- **PIN Verification**: 6-digit PIN verification using an explicit QR credential hash
- **Encrypted History**: Local Room storage is opened through SQLCipher and excluded from backups
...
- **SQLCipher + Security Crypto**: Encrypted local storage for scanned medical history
...
3. **PIN Verification** (if QR contains a PIN credential):
   - A popup dialog will appear requesting a 6-digit PIN
   - Enter the PIN generated by HospiSafe Desktop
...
When scanning a QR code that contains patient information, the app requires an explicit PIN credential:

1. Detect a PIN hash field in the JSON data (supports: `pin_hash`, `pinHash`, `access_pin_hash`, `accessPinHash`, `verification_hash`, `verificationHash`)
2. Show a PIN verification dialog
3. Verify the entered 6-digit PIN against the SHA-256 hash in the QR
4. Only show the patient information if the PIN is correct

For development fixtures only, the scanner also accepts a plain `pin`, `access_pin`, or `accessPin` field and hashes it in memory. Production QRs should provide only the hash.
...
```
For this example, the correct PIN is generated outside the scanner and validated against `pin_hash`.
